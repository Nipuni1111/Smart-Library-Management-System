...
const DamageReport = () => {
  ...
  useEffect(() => {
    if (bookSearch.trim().length < 2) { setBookResults([]); setShowDrop(false); return; }
    const t = setTimeout(async () => {
      setBookSearchLoading(true);
      try {
        const { data } = await api.get(`/books?search=${encodeURIComponent(bookSearch.trim())}`);
        setBookResults(data.slice(0, 8));
        setShowDrop(data.length > 0);
      } catch { setBookResults([]); setShowDrop(false); }
              ...
              <div className="form-row">
                <div className="form-group" style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    value={bookSearch}
                    onChange={e => { setBookSearch(e.target.value); sf('bookTitle', e.target.value); }}
                    onFocus={() => { if (bookResults.length) setShowDrop(true); }}
                    onBlur={() => setTimeout(() => setShowDrop(false), 180)}
                    placeholder="Search or type book title..."
                    autoComplete="off"
                  />
                  {bookSearchLoading && (
                    <span style={{ position: 'absolute', right: 12, top: '2.4rem', fontSize: '0.75rem', color: 'var(--text-muted)' }}>Searching…</span>
                  )}
                  {showDrop && bookResults.length > 0 && (
                    <div style={{
                      ...
                      {bookResults.map(b => (
                        <div key={b._id}
                          onMouseDown={() => {
                            ...
                            setForm(f => ({ ...f, bookTitle: b.title, bookIsbn: b.isbn || '' }));
                            if (errors.bookTitle) setErrors(p => ({ ...p, bookTitle: '' }));
                            setShowDrop(false);
                          }}
                          style={{ padding: '0.65rem 1rem', cursor: 'pointer', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}
                          onMouseEnter={e => e.currentTarget.style.background = 'var(--primary-bg)'}
                          onMouseLeave={e => e.currentTarget.style.background = ''}
                        >
                          <div>
                            <div style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text)' }}>{b.title}</div>
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{b.author}{b.isbn ? ` · ISBN: ${b.isbn}` : ''}</div>
                          </div>
                          {b.category && <span className="badge badge-primary" style={{ fontSize: '0.65rem', flexShrink: 0, marginLeft: '0.5rem' }}>{b.category}</span>}
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <div className="form-group">
                  ...
                  <input className="form-control" value={form.bookIsbn} onChange={e => sf('bookIsbn', e.target.value)} placeholder="e.g. 9780262046305" />
                </div>
                <div className="form-group">
