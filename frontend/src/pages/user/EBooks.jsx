              ...
              {displayBooks.map(ebook => (
                <div key={ebook._id} className="card" style={{ overflow: 'hidden' }}>
                  <div style={{ position: 'relative', height: 160, background: 'var(--primary-bg)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    {user && (
                      <button onClick={() => handleToggleFav(ebook._id)} style={{ position: 'absolute', top: 8, right: 8, background: 'rgba(255,255,255,0.85)', border: 'none', cursor: 'pointer', fontSize: '1.1rem', width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: favourites.includes(ebook._id) ? '#ef4444' : '#94a3b8', transition: 'var(--t)', boxShadow: '0 1px 4px rgba(0,0,0,0.2)', flexShrink: 0 }}>
                        ❤️
                      </button>
                    )}
                  </div>
